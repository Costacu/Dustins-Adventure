...
void GameEngine::update(float dt) {
    ...
    for (auto& entityPtr : entities_) {
        ...
        if (auto* enemy = dynamic_cast<Enemy*>(entityPtr.get())) {
            ...
            enemy->updateAI(dt, sf::Vector2f(player_.getPosition().getX(), player_.getPosition().getY()), map_, player_.isHidden());
    ...
    // Stergem entitatile inactive (doar Decoy-urile pot deveni inactive momentan)
    entities_.erase(std::remove_if(entities_.begin(), entities_.end(),
        [](const std::unique_ptr<Entity>& e) {
            if (const auto* decoy = dynamic_cast<const Decoy*>(e.get())) {
                return !decoy->active();
            }
            return false;
        }), entities_.end());

    // Iteram pentru logica de distragere (Enemy vs Decoy)
    for (auto& entEnemy : entities_) {
        if (auto* enemy = dynamic_cast<Enemy*>(entEnemy.get())) {
            ...
            for (const auto& entDecoy : entities_) {
                if (const auto* decoy = dynamic_cast<const Decoy*>(entDecoy.get())) {
...
void GameEngine::checkCollisions() {
    ...
    for (const auto& e : entities_) {
        if (const auto* enemy = dynamic_cast<const Enemy*>(e.get())) {
            sf::FloatRect eB = enemy->getBounds();
            if (pB.intersects(eB)) {
                gameOver_ = true;
                playerWon_ = false;
                updateOverlayText("Ai fost mancat de Demogorgoni!", "Apasa tasta R pentru a incerca din nou");
                return;
            }
        }
    }
...
void GameEngine::checkWinCondition() {
    ...
    if (map_.reachedWinDoor(player_.getBounds())) {
        gameOver_ = true;
        playerWon_ = true;
        updateOverlayText("Ai reusit sa iesi din Laboratorul din Hawkins!", "Apasa tasta R pentru a juca din nou");
    }
}
